...
use std::{fs};
use mpc_over_signal::{DeviceStore};
use tokio::runtime::Runtime;
use crate::cli as cli;
use cli::{keygen_run, signal_client, read_group, sign_run};
use std::vec::Vec;
...
use crate::common::party_i::{Keys, SharedKeys, LocalKeyShare};
...
pub fn keygen(secrets_byte_vec: Vec<u8>, group_byte_vec: Vec<u8>) -> Result<()> {
    ...
    rt.block_on(async {
        ...
        let mut device_secrets = DeviceStore::from_byte_vec(secrets_byte_vec)
            .await
...
////////////////////////Trigger after scan QR finished/////////////////////////////////////
